...
void BuildSimpleTemplates(TString folder="", int generated_PDF_set=1, int generated_PDF_member=0){
  ...
  TH1D hWlikePosSimpleTemplate[WMass::PDF_members][WMass::etaMuonNSteps][2*WMass::WMassNSteps+1];
  TH1D hWlikePosSimpleTemplate_NonScaled[WMass::PDF_members][WMass::etaMuonNSteps][2*WMass::WMassNSteps+1];
  ...
  for(int i=0; i<WMass::etaMuonNSteps; i++){
    ...
    for(int j=0; j<2*WMass::WMassNSteps+1; j++){
      ...
      for(int h=0; h<WMass::PDF_members; h++){
        cout << "retrieving histo " <<  Form("%s_pdf%d-%d_eta%s_%d",Zana_str[CutSteps-1].Data(),WMass::PDF_sets<0?generated_PDF_set:WMass::PDF_sets,h,eta_str.Data(),jWmass) << ", ";
        TH1D*hZ=(TH1D*)Zana->Get(Form("%s_pdf%d-%d_eta%s_%d",Zana_str[CutSteps-1].Data(),WMass::PDF_sets<0?generated_PDF_set:WMass::PDF_sets,h,eta_str.Data(),jWmass));
        ...
        hWlikePosSimpleTemplate[h][i][j]=(*hZ)*(*hR);
        // hWlikePosSimpleTemplate.Sumw2();
        hWlikePosSimpleTemplate[h][i][j].SetName(Form("hWlikePos_PtScaled_RWeighted_SimpleTemplates_pdf%d-%d_eta%s_%d",WMass::PDF_sets<0?generated_PDF_set:WMass::PDF_sets,h,eta_str.Data(),jWmass));
        hWlikePosSimpleTemplate[h][i][j].SetTitle(Form("hWlikePos_PtScaled_RWeighted_SimpleTemplates_pdf%d-%d_eta%s_%d",WMass::PDF_sets<0?generated_PDF_set:WMass::PDF_sets,h,eta_str.Data(),jWmass));
        hZ->Print();
        hR->Print();
        hWlikePosSimpleTemplate[h][i][j].Print();

        cout << "retrieving histo " <<  Form("hWlikePos_PtNonScaled_8_JetCut_pdf%d-%d_eta%s_%d",WMass::PDF_sets<0?generated_PDF_set:WMass::PDF_sets,h,eta_str.Data(),jWmass) << ", ";
        TH1D*hZ_PtNonScaled=(TH1D*)Zana->Get(Form("hWlikePos_PtNonScaled_8_JetCut_pdf%d-%d_eta%s_%d",WMass::PDF_sets<0?generated_PDF_set:WMass::PDF_sets,h,eta_str.Data(),jWmass));
        ...
        hWlikePosSimpleTemplate_NonScaled[h][i][j]=(*hZ_PtNonScaled)*(*hR_PtNonScaled);
        // hWlikePosSimpleTemplate_NonScaled.Sumw2();
        hWlikePosSimpleTemplate_NonScaled[h][i][j].SetName(Form("hWlikePos_PtNonScaled_RWeighted_SimpleTemplates_pdf%d-%d_eta%s_%d",WMass::PDF_sets<0?generated_PDF_set:WMass::PDF_sets,h,eta_str.Data(),jWmass));
        hWlikePosSimpleTemplate_NonScaled[h][i][j].SetTitle(Form("hWlikePos_PtnonScaled_RWeighted_SimpleTemplates_pdf%d-%d_eta%s_%d",WMass::PDF_sets<0?generated_PDF_set:WMass::PDF_sets,h,eta_str.Data(),jWmass));
        hZ_PtNonScaled->Print();
        hR_PtNonScaled->Print();
        hWlikePosSimpleTemplate_NonScaled[h][i][j].Print();
  ...
  for(int i=0; i<WMass::etaMuonNSteps; i++){
    for(int j=0; j<2*WMass::WMassNSteps+1; j++){
      for(int h=0; h<WMass::PDF_members; h++){
        ...
        hWlikePosSimpleTemplate[h][i][j].Write();
        hWlikePosSimpleTemplate_NonScaled[h][i][j].Write();
      // cWtemplate[j]->Write();
      }
    }
  }
