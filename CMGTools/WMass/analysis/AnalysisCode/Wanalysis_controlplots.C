#define Wanalysis_controlplots_cxx
...
void Wanalysis_controlplots::Loop(int IS_MC_CLOSURE_TEST, int isMCorDATA, TString outputdir, int useMomentumCorr, int smearRochCorrByNsigma, int useEffSF, int usePtSF, int useVtxSF, int controlplots, TString sampleName, int generated_PDF_set, int generated_PDF_member, int contains_PDF_reweight)
{
  ...
  cout << "output filename= " << Form("%s/Wanalysis_controlplots.root",outputdir.Data()) << endl;
  
  ofstream outTXTfile;
  outTXTfile.open(Form("%s/Wanalysis_controlplots_EVlog.log",outputdir.Data()));
  ...
  TFile*fout = new TFile(Form("%s/Wanalysis_controlplots.root",outputdir.Data()),"RECREATE");
  fout->cd();
  for(int h=0; h<WMass::PDF_members; h++)
