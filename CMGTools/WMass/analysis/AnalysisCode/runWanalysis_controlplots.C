...
#include <TROOT.h>
#include "Wanalysis_controlplots.h"
...
int main(int argc, char ** argv) 
{
  ...
  cout << "processing line "<< Form("Wanalysis_controlplots wDATA(\"%s\",%f,%d)",WfileDATA.Data(),WfileDATA_lumi_SF,sample.Contains("WJetsSig")?useAlsoGenPforSig:0) << endl;
  Wanalysis_controlplots wDATA(WfileDATA.Data(),WfileDATA_lumi_SF,sample.Contains("WJetsSig")?useAlsoGenPforSig:0);
  ...
  gROOT->ProcessLine(Form(".! mv ../%s/Wanalysis_controlplots.root ../%s/Wanalysis_controlplotsOnDATA.root",filename_outputdir.Data(),filename_outputdir.Data()));
  gROOT->ProcessLine(Form(".! cp Wanalysis_controlplots.* ../%s/",filename_outputdir.Data()));

}

// void runWanalysis_controlplots(TString WfileDATA, double WfileDATA_lumi_SF, TString sample, int useAlsoGenPforSig, int IS_MC_CLOSURE_TEST, int isMCorDATA, TString filename_outputdir, int useMomentumCorr, int smearRochCorrByNsigma, int useEffSF, int usePileupSF, int controlplots)
// {

  // // gSystem->CompileMacro("MuScleFitCorrector.cc");
  // gSystem->CompileMacro("rochcor_44X_v3.C");
  // // gSystem->CompileMacro("rochcor_42X.C");
  // gSystem->CompileMacro("Wanalysis_controlplots.C");
  // cout << "processing line "<< Form("Wanalysis_controlplots wDATA(\"%s\",%f,%d)",WfileDATA.Data(),WfileDATA_lumi_SF,sample.Contains("WJetsSig")?useAlsoGenPforSig:0) << endl;
  // // gROOT->ProcessLine(Form("Wanalysis_controlplots wDATA(\"%s\",%f,%d); wDATA.Loop(%d,%d,\"../%s\",%d,%d,%d,%d,\"%s\")",WfileDATA.Data(),WfileDATA_lumi_SF,sample.Contains("WJetsSig")?useAlsoGenPforSig:0,IS_MC_CLOSURE_TEST,isMCorDATA,filename_outputdir.Data(),useMomentumCorr,smearRochCorrByNsigma,useEffSF,usePileupSF,sample.Data()));
  // gROOT->ProcessLine(Form("Wanalysis_controlplots wDATA(\"%s\",%f,%d)",WfileDATA.Data(),WfileDATA_lumi_SF,sample.Contains("WJetsSig")?useAlsoGenPforSig:0));
  ...
  // // if(IS_MC_CLOSURE_TEST && sample.Contains("WJetsSig")){
    ...
    // // gROOT->ProcessLine(Form(".! mv ../%s/Wanalysis_controlplots.root ../%s/Wanalysis_controlplotsOnDATA.root",filename_outputdir2.Data(),filename_outputdir2.Data()));
  // // }
